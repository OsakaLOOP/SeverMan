...
支付使用爱发电官方 Open API、RSA Webhook 和 OAuth2 关联，金额采用最小货币单位整数。套餐公开资料通过 API 同步，权限使用本地版本化权益规则；Webhook 持久接收后经商户 API 复核，核销事务统一写入订单与权益。每 15 分钟增量检查、每日完整对账和用户主动同步共同更新订阅。模型、协议与部署标准见 [爱发电支付](billing.md)。
