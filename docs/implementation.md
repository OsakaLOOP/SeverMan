...
状态：已完成主流程、面板与部署配置，进入商户联调。

完成套餐公开资料同步、本地版本化权益、中心付款标识、RSA Webhook 接收、API 复核、OAuth2 关联、订单核销、用户分页查询、定期完整对账及失败重试。跨站授权通过最小权益视图直接读取，用户同步任务等待所有分页及核销子任务结束。设计与验收入口为 [爱发电支付](billing.md)。

开发路线：第一步完成模拟 API 与真实 PostgreSQL 验证；第二步在 Ubuntu 配置商户凭据和真实套餐，完成一次支付与 OAuth2 联调；第三步将各子站会员功能接入 `sdk/entitlements.ts`，按实际内容范围设置权益 key；商品 SKU、自动发码与平台侧退款操作随后按业务需求扩展。

用户已确认无既有支付记录，支付模块从空数据库表开始。RailRound 其他业务迁移独立推进。

本阶段验证：真实 PostgreSQL 集成测试、TypeScript 检查与生产构建；爱发电专项覆盖官方签名向量、通知去重、未签名字段复核、OAuth2、分页恢复、任务树等待、续费规则快照、增量对账、过期和读取权限。桌面 1440×1000 与移动端 390×844 检查套餐、分页、核销及进度交互。真实商户通信与 Ubuntu 性能按支付文档验收。
